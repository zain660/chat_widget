...
(function ($) {
    ...
    $.fn.chatBoxInit = function (app_key) {

        function after_append_run_firebase(session) {
       
            var appKey = app_key.app_key; // Replace this with the actual way you retrieve app_key
            ...
            reff.on('child_added', function (snapshot) {
                if(snapshot.val().user_id == sessionId){
                    var visitor_message_bubble = '<div class="d-flex flex-row justify-content-end mb-4 pt-1"><div><p class="small p-2 me-3 mb-3 text-white rounded-3 bg-warning">'+snapshot.val().text+'</p></div><img src="http://127.0.0.1:8000/assets/media/avatar/avatar.png" alt="avatar 1" style="width: 45px; height: 100%;"></div>';

                    $('#message_append').append(visitor_message_bubble);
                }else{
                    var agent_message_bubble = '<div class="d-flex flex-row justify-content-start"><img src="http://127.0.0.1:8000/assets/media/avatar/avatar.png" alt="avatar 1" style="width: 45px; height: 100%;"><div><p class="small p-2 ms-3 mb-3 rounded-3 bg-body-tertiary">'+snapshot.val().text+'</p></div></div>';
                    $('#message_append').append(agent_message_bubble);
                }
        ...
        var cdnScripts = [
            ...
            "https://www.gstatic.com/firebasejs/7.23.0/firebase.js"
        ];

        // Load all necessary scripts
        loadScriptsSequentially(cdnScripts);
        ...
        // Ajax request to get chat box HTML
        $.ajax({
            type: 'GET',
            url: 'http://127.0.0.1:8000/append_chat/' + app_key.app_key +'/?domain_url='+window.location.href+' ',
            crossDomain: true,
            ...
            _success: function (response) {
                ...
                $('body').append(response.chatcode);
                var currentUrl = window.location.href;
                
                $('.web_url').val(currentUrl);
                if (getSessionCookie('session_id') != null) {
                    $('#info_form_div').css("display", "none");
                    $('#messages').css("display", "block");
                    $('#session_id').val(getSessionCookie('session_id'))
                } else {
                    $('#info_form_div').css("display", "block");
                    $('#messages').css("display", "none");
                }

                after_append_run_firebase(getSessionCookie('session_id'));
                ...
                $('#message_form').submit(function (e) {
                    ...
                    $.ajax({
                        url: 'http://127.0.0.1:8000/message_send_from_visitor',
                        type: 'POST',
                        data: form,
                        processData: false,
                        contentType: false,
                        ...
                        success: function (response) {
                            ...
                            // after_append_run_firebase(getSessionCookie('session_id'));
                            $("#message_append").animate({
                                scrollTop: $('#message_append').get(0).scrollHeight
                            }, 0);
                        }
                    });
                });
                ...
                $('#info_form').submit(function (e) {
                    e.preventDefault();
                    var form = new FormData(document.getElementById('info_form'));
                    form.append('_token', $('input[name="_token"]').val());
                    $('#web_url').val(window.location.href);
                  
                    const randomString = generateRandomString(10); // Generate a random string of length 10

                    setSessionCookie('session_id', '' + randomString + '', 30);
                    const sessionId = getSessionCookie('session_id');

                    $.ajax({
                        url: 'http://127.0.0.1:8000/create_leads?session_id='+sessionId+' ',
                        type: 'POST',
                        data: form,
                        processData: false,
                        contentType: false,
                        ...
                        success: function (response) {
                            ...
                            document.getElementById('info_form_div').style.display = "none";
                ...
                // Show chat container on button click
